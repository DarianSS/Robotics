...
void setLinkedList(total_d_x, total_d_y, final_angle)
{
	new = create();
...
void move()
{
	while (1){
		...
		setLinkedList(total_d_x, total_d_y, final_angle);
	}
}
...
void target_point(coords *ahead)
{
	if (sqrt((total_d_x - current->x) + (total_d_y - current->y)) >= 15) 
	{
		speed=new_speed2(current->angle-ahead->angle);
		set_motors(speed,SPEED);
...
void trace()
{
	coords *ahead = current;
	while(ahead)
	{
		//printf("Coordinates: X=%d Y=%d\n", current->x, current->y);set_point(current->x, current->y);
		target_point(ahead);
		set_motors(0,0);
	}
}
